...
GEE_PROJECT_ID = st.secrets["GEE_PROJECT_ID"]
...
def get_satellite_image(lat, lon, size=100, scale=30):
    ...
    try:
        if not ee.data._initialized:
            ...
            mock_img = np.random.rand(size, size, 3) * 0.3
            mock_img[:, :, 1] += 0.6  # Add more green for vegetation
            return mock_img
...
class EnhancedFireSimulator:
    def __init__(self, lat, lon, size=100, scale=30):
        ...
        # State tracking
        self.burned_cells = 0
        self.burning_cells = np.sum(self.grid >= 0.8)
        self.time_elapsed = 0
        self.history = []
